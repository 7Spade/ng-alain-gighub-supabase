...
| `documentation-writer.prompt.md` | Diátaxis 文件專家 | 依 Diátaxis 框架撰寫技術文件 |
| `create-oo-component-documentation.prompt.md` | OO 元件文件產出 | 物件導向元件完整文件 |
| `update-oo-component-documentation.prompt.md` | OO 元件文件更新 | 更新現有 OO 元件文件 |
| `comment-code-generate-a-tutorial.prompt.md` | 程式碼教學產出 | 將程式碼轉換為入門教學 |
| `add-educational-comments.prompt.md` | 教育性註解新增 | 為程式碼新增教育性註解 |
...
| `create-technical-spike.prompt.md` | 技術驗證文件 | 時間盒內的技術研究文件 |
...
| `breakdown-test.prompt.md` | 測試規劃 | 全面測試策略與品質驗證計畫 |
...
| `my-issues.prompt.md` | 我的 Issues | 列出目前倉庫中我的 Issues |
| `my-pull-requests.prompt.md` | 我的 PRs | 列出目前倉庫中我的 Pull Requests |
| `gen-specs-as-issues.prompt.md` | 規格轉 Issues | 識別缺失功能並建立詳細規格 |
| `git-flow-branch-creator.prompt.md` | Git Flow 分支 | 建立 Git Flow 分支 |
| `conventional-commit.prompt.md` | 傳統提交訊息 | 產生符合 Conventional Commits 的提交訊息 |
...
| 提示檔案 | 描述 | 適用場景 |
|----------|------|----------|
| `playwright-generate-test.prompt.md` | Playwright 測試產出 | 基於場景產生 Playwright 測試 |
| `playwright-explore-website.prompt.md` | 網站探索 | 使用 Playwright MCP 探索網站 |
...
| `sql-optimization.prompt.md` | SQL 通用最佳化 | 跨資料庫 SQL 效能最佳化 |
| `sql-code-review.prompt.md` | SQL 通用程式碼審查 | 跨資料庫 SQL 安全性與品質審查 |
...
| `write-coding-standards-from-file.prompt.md` | 編碼標準文件 | 從程式碼檔案產出編碼標準文件 |
| `generate-custom-instructions-from-codebase.prompt.md` | 自訂指令產出 | 分析版本差異產出遷移指令 |
...
| 提示檔案 | 描述 | 適用場景 |
|----------|------|----------|
| `multi-stage-dockerfile.prompt.md` | 多階段 Dockerfile | 產出最佳化的多階段 Dockerfile |
| `editorconfig.prompt.md` | EditorConfig 產出 | 根據專案分析產出 .editorconfig |
| `create-llms.prompt.md` | llms.txt 產出 | 根據倉庫結構產出 llms.txt |
| `update-llms.prompt.md` | llms.txt 更新 | 更新 llms.txt 反映文件變更 |
...
| 提示檔案 | 描述 | 適用場景 |
|----------|------|----------|
| `repo-story-time.prompt.md` | 倉庫故事 | 從提交歷史產出倉庫摘要與故事 |
| `shuffle-json-data.prompt.md` | JSON 資料洗牌 | 安全地洗牌 JSON 陣列資料 |
| `update-markdown-file-index.prompt.md` | Markdown 索引更新 | 更新 Markdown 檔案的檔案索引 |
| `create-agentsmd.prompt.md` | AGENTS.md 產出 | 產出倉庫的 AGENTS.md 檔案 |
| `github-copilot-starter.prompt.md` | Copilot 入門 | GitHub Copilot 入門指引 |
| `javascript-typescript-jest.prompt.md` | Jest 測試 | JavaScript/TypeScript Jest 測試 |
| `typescript-mcp-server-generator.prompt.md` | TypeScript MCP 產出 | 產出完整的 TypeScript MCP 伺服器專案 |
| `mkdocs-translations.prompt.md` | MkDocs 翻譯 | MkDocs 文件堆疊的語言翻譯 |
...
- **2025-11-26**: 清理非 Angular/通用目的的提示，建立完整 README 文件
  ...
  - 保留：通用開發、文件產出、架構規劃、GitHub 整合、Playwright 測試、PostgreSQL、Prompt 工程等相關提示
