...
export class UserBlueprintsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.route.params.subscribe(params => {
      if (params['userId']) {
        ...
        this.loadBlueprints(params['userId']);
      }
    });
  }

  /**
   * Load blueprints for user
   */
  private async loadBlueprints(userId: string): Promise<void> {
    try {
  ...
  onCreateBlueprint(): void {
    ...
    this.modal.createStatic(BlueprintCreateModalComponent, { ownerId: userId, ownerType: 'user' }, { size: 'md' }).subscribe(result => {
      if (result) {
        this.message.success('藍圖建立成功');
        // Reload blueprints
        this.loadBlueprints(userId);
      }
    });
  }
