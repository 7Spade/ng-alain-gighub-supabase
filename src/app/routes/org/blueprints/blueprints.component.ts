  ...
  template: `
    ...
    <nz-card [nzTitle]="'組織藍圖列表'">
      ...
      @if (!loading() && filteredBlueprints().length === 0) {
        <nz-empty [nzNotFoundContent]="'尚無組織藍圖'" [nzNotFoundFooter]="emptyFooter">
          <ng-template #emptyFooter>
            <button nz-button nzType="primary" (click)="onCreateBlueprint()"> 建立第一個組織藍圖 </button>
          </ng-template>
        </nz-empty>
      }
            ...
            <nz-col [nzSpan]="8">
              ...
              >
                ...
                <div class="blueprint-meta">
                  ...
                  <nz-tag [nzColor]="'gold'">{{ blueprint.visibility }}</nz-tag>
                  @if (blueprint.usageCount !== undefined) {
                    <nz-tag [nzColor]="'green'">使用次數: {{ blueprint.usageCount }}</nz-tag>
                  }
                ...
                <div class="blueprint-actions mt-md">
                  ...
                  @if (blueprint.status === 'draft') {
                    <button nz-button nzType="default" nzSize="small" (click)="onPublishBlueprint(blueprint); $event.stopPropagation()">
...
export class OrgBlueprintsComponent implements OnInit, OnDestroy {
  ...
  onCreateBlueprint(): void {
    this.message.info('新增組織藍圖功能即將推出');
  ...
  /**
   * Handle publish blueprint
   */
  async onPublishBlueprint(blueprint: BlueprintModel): Promise<void> {
    try {
      await this.blueprintFacade.publishBlueprint(blueprint.id);
      this.message.success('藍圖已成功發布');
      await this.loadBlueprints(); // Reload to show updated status
    } catch (error) {
      this.message.error('發布藍圖失敗');
